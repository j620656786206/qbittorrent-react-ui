      ...
      '/api': {
        ...
        configure: (proxy, _options) => {
          proxy.on('proxyReq', (proxyReq, req, _res) => {
            console.log(`[Vite Proxy] Forwarding Request: ${req.method} ${proxyReq.protocol}//${proxyReq.host}${proxyReq.path}`);
            console.log('[Vite Proxy] Outgoing Request Headers:', JSON.stringify(proxyReq.getHeaders(), null, 2));
            ...
            if (req.url && req.url.includes('/api/v2/auth/login')) {
              ...
              console.log('[Vite Proxy] Cookie header removed for login request.');
            }
          });
